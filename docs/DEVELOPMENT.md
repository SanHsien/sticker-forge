...
目前已建立最小 Python CLI、核心模組、原生 GUI、本機 HTML fallback 與 Windows `.exe` 打包流程。

短期任務：

- 擴充圖片處理核心：預覽資料與互動選圖。
- 擴充 ZIP 匯出前錯誤提示。
- 強化原生 GUI 的拖放、縮放預覽與 Windows installer。
...
- 版本：`v0.2.0`
- 主程式：`sticker-forge.exe`，原生 GUI，無 console。
- 命令列：`sticker-forge-cli.exe`，保留 CLI。
- 產物：`sticker-forge-v0.2.0-windows-x64.zip`
- checksum：`sticker-forge-v0.2.0-windows-x64.zip.sha256`
...
- GUI 產物在 `%TEMP%\sticker-forge-pyinstaller-dist\sticker-forge\sticker-forge.exe`。
- CLI 產物在 `%TEMP%\sticker-forge-pyinstaller-dist\sticker-forge\sticker-forge-cli.exe`。
- 跑 `sticker-forge.exe --smoke` 與 `sticker-forge-cli.exe --help` smoke test。
