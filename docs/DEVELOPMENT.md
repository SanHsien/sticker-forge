...
| `exporter` | LINE ZIP、PNG-only ZIP、多平台 ZIP（`PLATFORM_SPECS`：Telegram/WhatsApp/Discord/Signal）匯出與 ZIP 驗證（含透明背景檢查）、尺寸整理與 padding |
...
已發行：`v0.1.0`…`v0.7.0`。exe 圖示為 `packaging/icon.ico`。
