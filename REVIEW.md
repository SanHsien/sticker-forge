...
`sticker-forge` v0.17.0 是可用的 local-first 貼圖工具：CLI 與 pywebview 桌面 GUI 共用 Python core，支援 LINE 靜態貼圖、Big Stickers、emoji、訊息貼圖、動態貼圖、pop-up stickers、effect stickers 與多平台尺寸匯出。專案仍符合不架 server、不代管 AI API、不上傳使用者圖片、不自動送 LINE 審核的邊界。

v0.13.1 已補發 Windows exe，讓 GUI 匯出錯誤顯示修正進入正式 Release。v0.14.0 補齊一般使用者指南與可重現範例素材流程。v0.15.0 補上 Signal manifest pack 與 `validate --signal`。v0.16.0 補上 LINE Big Stickers 匯出與 `validate --big`。v0.17.0 補上 LINE pop-up / effect stickers CLI 匯出與 validator。
...
- GitHub Release 含 `sticker-forge-v0.17.0-windows-x64.zip` 與 `.sha256`。
...
### P2：pop-up / effect 目前是 CLI-first

v0.17.0 已有核心與 CLI，但 GUI 尚未提供 pop-up / effect 的多動畫檔匯入、預覽與匯出入口。下一輪應優先補 GUI，否則一般使用者仍要看文件跑命令列。
